...
For `'youtube'`, `src` is the **video ID only** — `XXbsTmILF84`, not the full
...
## Before the reveal
This is a surprise. `PRELAUNCH` in `src/data/site.ts` keeps the site out of
Google — it drives both the `noindex` meta tag and `robots.txt` (generated by
`src/pages/robots.txt.ts`, not a static file). The URL keeps working for anyone
you send it to. On the day, set it to `false` and push; nothing else changes.

Note this only covers the site. **The GitHub repo is public**, and its
description gives the surprise away in one line.
...
If a push silently never builds — no entry at all in Vercel's deployment list,
not even a red one — GitHub dropped the webhook delivery. The integration is
fine; that one delivery was lost. `git commit --allow-empty -m "Trigger deploy"`
and push kicks it, and the empty commit carries the stranded commits with it.
...
## Privacy
Self-hosted fonts, `mailto:` instead of a form, and click-to-load for anything
third-party — no embed contacts YouTube until a visitor clicks it. The one
third-party script is **GoatCounter** (cookieless, production only), and
`datenschutz.astro` declares it. Keep the page and the code in step: anything
added here has to be declared there too.
